# Self-Federated Anomaly Detection

This repository contains materials related to the Self-Federated Anomaly Detection (SFAD) project, including a visual diagram and the accompanying research paper.

## Contents

- [diagram.png](diagram.png) — architecture / workflow diagram for the system
- [Research paper SFAD system.pdf](Research%20paper%20SFAD%20system.pdf) — reference paper for the project

## Overview

Self-federated anomaly detection focuses on identifying unusual or suspicious behavior in a distributed setting while keeping the learning process aligned with federated and privacy-aware constraints.

## Diagram

![System diagram](diagram.png)

## Notes

- Open the PDF file to review the full research background.
- The diagram provides a visual summary of the system flow and main components.
